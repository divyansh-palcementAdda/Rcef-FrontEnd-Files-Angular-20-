...
export class Test3 implements OnInit, AfterViewInit {
  taskForm!: FormGroup;
  ...
  private initForm(): void {
    ...
    this.taskForm.get('departmentIds')?.valueChanges.subscribe((deptIds) => {
      this.updateFilteredUsers();
      if (deptIds.length > 0) {
        deptIds.forEach((id: number) => {
          if (!this.expandedDepts[id]) {
            this.expandedDepts[id] = true;
          }
        });
      }
    });
  ...
  updateDepartmentSelection(deptId: number, checked: boolean): void {
    ...
    if (checked && !selected.includes(deptId)) {
      ...
      delete this.expandedDepts[deptId];
    }
  ...
  toggleDepartmentExpansion(deptId: number): void {
  ...
  onDepartmentCardClick(dept: Department, event: MouseEvent): void {
    if ((event.target as HTMLElement).tagName === 'INPUT') {
      return;
    }

    const isDisabled = this.taskForm.value.assignToSelf && 
                      !this.currentUser?.departmentIds?.includes(dept.departmentId);
    
    if (isDisabled) return;

    const isSelected = this.taskForm.value.departmentIds.includes(dept.departmentId);
    this.updateDepartmentSelection(dept.departmentId, !isSelected);
  }

  onUserItemClick(deptId: number, user: userDto, event: MouseEvent): void {
    if ((event.target as HTMLElement).tagName === 'INPUT') {
      return;
    }
    ...
    const isDisabled = this.taskForm.value.assignToSelf 
      ...
      : this.isUserSelectionDisabled(user);
    
    if (isDisabled) return;

    const isSelected = this.selectedUsersByDeptObj[deptId]?.includes(user.userId) || false;
    this.updateUserSelection(deptId, user.userId, !isSelected);
  }
  ...
  onSubmit(): void {
    ...
    if (!departmentIds?.length) {
      ...
      this.setActiveSection('departments');
      return;
    }
  ...
  calculateFormCompletion(): number {
    const totalFields = 6;
    let completed = 0;
    
    const values = this.taskForm.value;
    
    if (values.title?.trim()) completed++;
    if (values.status) completed++;
    if (values.dueDate) completed++;
    if (values.departmentIds?.length > 0) completed++;
    if (values.description?.trim()) completed++;
    
    if (values.status === 'UPCOMING') {
      if (values.startDate) completed++;
    }
    
    return Math.round((completed / (values.status === 'UPCOMING' ? totalFields : totalFields - 1)) * 100);
  }
